...
export function KpiTable({ rows }: Readonly<KpiTableProps>) {
  return (
    <section className="border-b border-border/80 px-2 pb-10 sm:px-4 lg:px-6">
      <div className="mb-5">
        <h3 className={SECTION_TITLE_CLASS_NAME}>All KPIs</h3>
      </div>
      {rows.length === 0 ? <p className="text-sm text-muted-foreground">Run a backtest to populate KPI metrics.</p> : null}
      {rows.length > 0 ? (
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={row.key} className="border-border/80">
                ...
                <TableCell className="py-3 text-right text-[0.98rem] font-medium text-foreground">{row.value}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
